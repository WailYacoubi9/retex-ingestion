...
RESUMES = json.load(open("resumes.json", encoding="utf-8")) if os.path.exists("resumes.json") else {}


def resume(r):
    """Résumé LLM exporté de Neo4j, matché par Num F.E.. '' si absent."""
    return clean(RESUMES.get(clean(r.get("Num F.E.")) or "", "")) or ""
...
def texte(r, var, champs_d):
    ...
    if var == "A":
        return n
    if var == "E":                       # D + résumé, MÊME chunk
        base = texte(r, "D", champs_d)
        rs = resume(r)
        return (base + "\n" + rs).strip() if rs else base
...
def main():
    ...
    # F : D + résumé en DEUX vecteurs par fiche (chunk séparé) — mesure type A0
    vchF = {}
    for i in idx:
        parts = [embed(texte(D[i], "D", champs_d))]
        rs = resume(D[i])
        if rs:
            parts.append(embed(rs))
        vchF[i] = parts
    res["F"] = mesure_a0(idx, cible, vchF)
    pickle.dump(_cache, open(CACHE, "wb"))
    print("  [F] terminé            ")
    ...
    LIB = {"A0": "A0 · chunks par champ (ACTUEL)", "A": "A  · narratif fusionné",
           ...
           "E": "E  · D + résumé (même chunk)", "F": "F  · D + résumé (chunk séparé)"}
    print("\n" + "=" * 84)
    print(f"{'variante':34s} {'rappel@10 [IC95]':>26s} {'séparation':>12s} {'homogén.':>9s}")
    print("-" * 84)
    out = {}
    for v in ("A0", "A", "B", "C", "D", "E", "F"):
        rap, sep, homo = res[v]
        rm, rlo, rhi = boot(rap)
        sm, _, _ = boot(sep)
    ...
    print("\n--- RÉSUMÉ LLM indexé ? (E/F vs D, test apparié) ---")
    for v, desc in (("E", "même chunk"), ("F", "chunk séparé")):
        dv, lov, hiv = boot_apparie(res[v][0], res["D"][0])
        verdict = "UTILE" if lov > 0 else ("NUISIBLE" if hiv < 0 else "équivalent -> n'apporte rien")
        print(f"  {v} - D ({desc}) : {100*dv:+5.1f} pt  IC95 [{100*lov:+5.1f};{100*hiv:+5.1f}]  "
              f"| homogén. {out[v]['homogeneite']:.3f} (D={out['D']['homogeneite']:.3f})  -> {verdict}")
